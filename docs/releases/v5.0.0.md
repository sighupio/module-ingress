...
### HAProxy Kubernetes Ingress Controller v3.2.4
...
4. Update your `kustomization.yaml` to include the `external-dns/namespace` resource alongside the external-dns variants:

```yaml
resources:
  - <vendor-path>/katalog/external-dns/namespace
  - <vendor-path>/katalog/external-dns/public
  # If needed, also include:
  - <vendor-path>/katalog/external-dns/private
```
...
6. Verify the new deployment is running:
...
2. Update your `kustomization.yaml` to include the `external-dns/namespace` resource alongside the external-dns variants:
...
4. Verify the new deployment is running:
